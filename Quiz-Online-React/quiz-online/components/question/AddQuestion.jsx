           ...
           <div className='card-body'>
              <form onSubmit={handleSubmit} className='p-2'>
                 ...
                 <div className='mb-2'>
                    ...
                    <textarea className='form-control' rows={4} value={question} onChange={(e) => setQuestion(e.target.value)}>
                    </textarea>
                 </div>
                 <div className='mb-3'>
                 ...
                 {choices.map((choice,index) =>(
                    <div key={index} className='input-group mb-3'>
                       <input type='text' value={choice} className='form-control'
                             onChange={(e)=> handleChoiceChange(index,e.target.value)}/>
                      <button type='button' className='btn btn-outline-danger btn-sm'
                                        onClick={()=>handleRemoveChoice(index)}>Remove</button>
                    </div>
                 ))}
                 <button type='button' className='btn btn-outline-primary btn-sm'
                                        onClick={handleAddChoice}>Add Choice</button>
                 </div>
                 {questionType === "single" &&(
                   <div className='mb-3'>
                    <label htmlFor='answer' className='form-label text-info'>Correct Answer</label>
                    <input type='text' value={correctAnswers[0]} className='form-control'
                    ...
                    {correctAnswers.map((answer,index) =>(
                      <div>
                        <input type='text' value={correctAnswers[0]} className='form-control'
                             onChange={(e)=> handleCorrectAnswerChange(0,e.target.value)}/>
                      {index >0 && (
                        <button type='button' className='btn btn-danger btn-sm' onClick={()=>handleRemoveCorrectAnswer(index)}>
                            Remove
                        </button>
                      )}
                      </div>
